#!/bin/bash
# Script to upload vascular MCQs to the correct Heroku app
...
LOG_FILE="${CURRENT_DIR}/vascular_upload_${TIMESTAMP}.log"
...
for CHUNK_FILE in "$CHUNK_DIR"/*.json; do
    ...
    heroku run "python /tmp/mcq_upload/import_mcqs.py /tmp/mcq_upload/$CHUNK_NAME" --app "$APP_NAME" 2>&1 | tee -a "$LOG_FILE"
...
heroku run "python -c \"from mcq.models import MCQ; print(f'Final Total MCQs: {MCQ.objects.count()}'); print(f'Final Vascular MCQs: {MCQ.objects.filter(subspecialty=\\\"vascular_neurology\\\").count()}');\"" --app "$APP_NAME" 2>&1 | tee -a "$LOG_FILE"
