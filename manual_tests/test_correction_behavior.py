...
# Ensure OpenAI API key is available from the environment before importing the processor.
if not os.environ.get("OPENAI_API_KEY") and not os.environ.get("OPENAI_KEY"):
    raise RuntimeError(
        "OPENAI_API_KEY environment variable must be set before running this manual test."
    )
