...
def ai_edit_mcq_options(request, mcq_id):
    ...
    try:
        ...
        try:
            ...
            logger.error(f"Failed to import ai_edit_options_direct: {e}", exc_info=True)
            return JsonResponse({
                'success': False,
                'error': f'Import error: {str(e)}'
            }, status=500)
        except AttributeError as e:
            logger.error(f"Function ai_edit_options_direct not found: {e}", exc_info=True)
            return JsonResponse({
                'success': False,
        ...
        if use_async:
            ...
            try:
                ...
                cache.set(f'ai_job:{job_id}', result, timeout=300)  # Cache for 5 minutes
                ...
                logger.error(f"Error processing options for MCQ #{mcq_id}: {str(e)}", exc_info=True)
                # Store error in cache
                error_result = {
                    'status': 'failed',
                    'error': str(e)
                }
                cache.set(f'ai_job:{job_id}', error_result, timeout=300)
