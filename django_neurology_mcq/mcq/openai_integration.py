...
_DEFAULT_FALLBACK_MODEL = os.environ.get('OPENAI_DEFAULT_FALLBACK_MODEL', '').strip() or "gpt-5-mini"
if _FALLBACK_FROM_ENV:
    FALLBACK_MODEL = _FALLBACK_FROM_ENV
elif DEFAULT_MODEL == "gpt-5-mini":
    FALLBACK_MODEL = _DEFAULT_FALLBACK_MODEL
else:
    FALLBACK_MODEL = DEFAULT_MODEL

# Specific model for options editing - using gpt-5-nano for faster response times
OPTIONS_MODEL = os.environ.get('OPENAI_OPTIONS_MODEL', '').strip() or "gpt-5-nano"
OPTIONS_FALLBACK_MODEL = os.environ.get('OPENAI_OPTIONS_FALLBACK_MODEL', '').strip() or "gpt-5-mini"

OPTION_REQUEST_TIMEOUT = int(os.environ.get("OPENAI_OPTION_REQUEST_TIMEOUT", "26"))


def _option_model_priority() -> List[str]:
    """Return option models in the order we should try them."""
    priority: List[str] = []

    # Use the specific OPTIONS_MODEL for options editing (gpt-5-nano for faster response)
    if OPTIONS_MODEL and OPTIONS_MODEL not in priority:
        priority.append(OPTIONS_MODEL)

    # Add the options fallback model next
    if OPTIONS_FALLBACK_MODEL and OPTIONS_FALLBACK_MODEL not in priority:
        priority.append(OPTIONS_FALLBACK_MODEL)

    # If neither options model works, fall back to the general models
    if DEFAULT_MODEL and DEFAULT_MODEL not in priority:
        priority.append(DEFAULT_MODEL)

    if FALLBACK_MODEL and FALLBACK_MODEL not in priority:
        priority.append(FALLBACK_MODEL)

    return priority or [OPTIONS_MODEL or DEFAULT_MODEL]
...
AGENT_BASE_DIR = Path(__file__).resolve().parent.parent.parent / "agents"
AGENT_SCRIPT_PATH = AGENT_BASE_DIR / "run_explanation_agent.js"
AGENT_NODE_MODULES = AGENT_BASE_DIR / "node_modules"
AGENT_TIMEOUT_SECONDS = int(os.environ.get("AI_AGENT_TIMEOUT", "25"))
AGENT_BACKGROUND_TIMEOUT_SECONDS = int(os.environ.get("AI_AGENT_BACKGROUND_TIMEOUT", "300"))  # 5 minutes for background tasks
AGENT_NPM_TIMEOUT_SECONDS = int(os.environ.get("AI_AGENT_NPM_TIMEOUT", "240"))
AGENT_ENABLED_DEFAULT = os.environ.get("MCQ_USE_AGENT_EXPLANATION", "1").lower() not in {"0", "false"}


def _ensure_agent_dependencies() -> None:
    """
    Install the @openai/agents dependency on-demand if it is not already present.
    """
    ...
    if not npm_path:
        raise RuntimeError("npm is required to run the OpenAI agent (npm not found on PATH).")

    if not AGENT_BASE_DIR.exists():
        raise RuntimeError(f"Agent workspace missing at {AGENT_BASE_DIR}")

    logger.info("Installing agent dependencies via npm (this may take a moment)...")
    result = subprocess.run(
        ...
        capture_output=True,
        text=True,
        timeout=AGENT_NPM_TIMEOUT_SECONDS,
    )

    if result.returncode != 0:
        stderr = result.stderr.strip() or "npm install failed without diagnostics."
        raise RuntimeError(stderr)


def _run_explanation_agent(prompt: str, timeout: int = None) -> str:
    """
    Execute the TypeScript agent workflow and return the generated explanation text.

    Args:
        prompt: The input prompt for the agent
        timeout: Optional timeout in seconds (defaults to AGENT_TIMEOUT_SECONDS)
    """
    if not AGENT_SCRIPT_PATH.exists():
        raise RuntimeError(
            f"Agent runner script not found at {AGENT_SCRIPT_PATH}. Did you commit agents/run_explanation_agent.js?"
        )

    _ensure_agent_dependencies()

    payload = json.dumps({"input_as_text": prompt})
    env = os.environ.copy()
    env.setdefault("NODE_NO_WARNINGS", "1")

    # Use provided timeout or default
    agent_timeout = timeout if timeout is not None else AGENT_TIMEOUT_SECONDS

    try:
        result = subprocess.run(
            ["node", str(AGENT_SCRIPT_PATH), payload],
            capture_output=True,
            text=True,
        ...
        logger.warning(f"Agent timeout after {agent_timeout}s - will fallback to direct API")
    ...
    if result.returncode != 0:
        ...
        message = error_payload.get("error") or stderr or "Agent runner failed without details."
    ...
    if not isinstance(output_text, str):
        raise RuntimeError("Agent response missing output_text field.")
    return output_text.strip()


def _build_explanation_agent_prompt(
    mcq,
    *,
    section_name: str,
    current_content: str,
    ...
    options_text = format_options_text(mcq) or "Options unavailable."
    correct_answer = getattr(mcq, "correct_answer", "") or "Unknown"
    subspecialty = getattr(mcq, "subspecialty", "General Neurology")
    explanation_reference = (
        current_content
        or getattr(mcq, "unified_explanation", "")
    ...
    prompt_sections = [
        "### Task",
        f"You are updating the '{section_name}' section of a neurology MCQ explanation.",
        "",
        "### MCQ Stem",
        question_text.strip() or "[No stem provided]",
        "",
        "### Answer Options",
        options_text.strip(),
        "",
        f"### Correct Answer (reference only)\n{correct_answer}",
        "",
        f"### Subspecialty Context\n{subspecialty}",
        "",
        "### Editing Mode",
        f"{mode}",
    ]
    ...
    prompt_sections.extend(
        [
            "",
            "### Requirements",
            "- Produce a board-review quality explanation tailored to neurology residents.",
            "- Explain why the correct answer is right and why each distractor is wrong.",
            "- Use concise markdown headings (###) for major sections.",
            "- Incorporate pathophysiology, distinguishing features, and management pearls with citations.",
            "- Mention diagnostic criteria and staging when relevant, confirming the latest update if referenced.",
        ]
    )
...
def generate_explanation_with_agent(
    mcq,
    *,
    section_name: str,
    current_content: str,
    ...
    prompt = _build_explanation_agent_prompt(
        mcq,
    ...
    timeout = AGENT_BACKGROUND_TIMEOUT_SECONDS if is_background else None
    output_text = _run_explanation_agent(prompt, timeout=timeout)
    if not output_text:
        raise RuntimeError("Agent returned empty explanation.")
    return output_text
...
def _extract_response_json(response: Any) -> Tuple[Dict[str, Any], str]:
    ...
    if output_items:
        for item in output_items:
            ...
            for block in content_list:
                if isinstance(block, dict):
                    # Check for both json_schema and output_json types (GPT-5 may use either)
                    if block.get("type") in ["json_schema", "output_json", "json"]:
                        json_payload = block.get("json") or block.get("data") or block.get("output")
                        if isinstance(json_payload, dict):
                            return json_payload, json.dumps(json_payload)
                else:
                    block_type = getattr(block, "type", None)
                    # Check for both json_schema and output_json types
                    if block_type in ["json_schema", "output_json", "json"]:
                        json_payload = getattr(block, "json", None) or getattr(block, "data", None) or getattr(block, "output", None)
                        if isinstance(json_payload, dict):
                            return json_payload, json.dumps(json_payload)
    ...
    **kwargs: Any,
    ...
    if hasattr(client, "responses"):
        ...
        if text_config:
            ...
            if is_gpt5 and isinstance(response_format, dict):
                ...
                # Use higher verbosity for JSON schema to avoid empty responses
                json_verbosity = "auto" if fmt_type == "json_schema" else default_verbosity
                responses_payload["text"] = {
                    "verbosity": json_verbosity,
    ...
    if responses_payload is not None:
        try:
            ...
            # Check if response has empty content (common with low verbosity GPT-5)
            # If empty, raise exception to trigger fallback to chat completions
            response_text = _extract_response_text(response)
            if not response_text or len(response_text.strip()) == 0:
                # Log more details about the response for debugging
                response_output = getattr(response, "output", None)
                logger.warning(f"Responses API returned empty content for model {model} (output type: {type(response_output)})")
                logger.warning(f"Response verbosity setting was: {responses_payload.get('text', {}).get('verbosity', 'unknown')}")
            ...
            # Fall through to chat completions for all models including GPT-5
            # Previously we raised for GPT-5, but this prevented fallback when Responses API
            # returns empty content with low verbosity. Now we allow fallback to chat completions.
    else:
...
def _fill_missing_options_with_model(mcq, custom_instructions: str, model_name: str) -> dict:
    """
    ...
    try:
        mcq_id = getattr(mcq, "id", "unknown")
        logger.info("AI filling missing options for MCQ #%s using model %s (gpt-5-nano for faster response)", mcq_id, model_name)
        ...
        for attempt in range(max_attempts):
            ...
            try:
                ...
                logger.warning("Attempt duration %.1fs before failure", time.monotonic() - start_time)
                if should_retry and attempt + 1 < max_attempts:
                    time.sleep(min(2 ** attempt, 3))
                    last_errors = [message]
                    continue
            ...
            logger.info(
                "Successfully filled missing options for MCQ #%s on attempt %s with model %s in %.1fs",
                mcq_id,
                attempt + 1,
                model_name,
        ...
        logger.error(
            "Error in ai_edit_options for MCQ #%s with model %s: %s",
...
def ai_edit_options(mcq, custom_instructions: str = "") -> dict:
    ...
    for model_name in _option_model_priority():
        try:
            return _fill_missing_options_with_model(mcq, custom_instructions, model_name)
        except ValueError as exc:
            last_error = exc
            logger.warning(
                "ai_edit_options failed for MCQ #%s using model %s: %s",
                getattr(mcq, "id", "unknown"),
                model_name,
                exc,
            )
    ...
    error_message = str(last_error) if last_error else "Failed to fill missing options."
    raise ValueError(error_message)


def _improve_all_options_with_model(mcq, custom_instructions: str, model_name: str) -> dict:
    """
    ...
    try:
        mcq_id = getattr(mcq, "id", "unknown")
        logger.info("AI improving all options for MCQ #%s using model %s", mcq_id, model_name)
        ...
        for attempt in range(max_attempts):
            ...
            logger.info(
                "Successfully improved all options for MCQ #%s on attempt %s with model %s in %.1fs",
                mcq_id,
                attempt + 1,
                model_name,
                time.monotonic() - start_time,
        ...
        should_retry, message, status_code = _classify_openai_error(e)
        logger.error(
            "Error in ai_improve_all_options for MCQ #%s with model %s: %s",
            getattr(mcq, "id", "unknown"),
            model_name,
            message,
        )
...
- Do NOT include any other text, just the options"""
    ...
    try:
        ...
        logger.info(f"Calling _responses_create with model: {DEFAULT_MODEL}")
...
def ai_improve_all_options(mcq, custom_instructions: str = "") -> dict:
    # TODO: Add caching here to avoid redundant API calls
    ...
    for model_name in _option_model_priority():
        try:
            return _improve_all_options_with_model(mcq, custom_instructions, model_name)
        except ValueError as exc:
            last_error = exc
            logger.warning(
                "ai_improve_all_options failed for MCQ #%s using model %s: %s",
                getattr(mcq, "id", "unknown"),
                model_name,
                exc,
            )
    ...
    error_message = str(last_error) if last_error else "Failed to improve options."
    raise ValueError(error_message)
...
def ai_edit_explanation_text(
    ...
    *,
    ...
    if AGENT_ENABLED_DEFAULT:
        try:
            return generate_explanation_with_agent(
                mcq,
            ...
            logger.error(
                "Agent-based explanation generation failed, falling back to legacy Responses API: %s",
                agent_error,
                exc_info=True,
            )
    if not api_key or not client:
    ...
    try:
        ...
        for attempt in range(max_attempts):
            ...
            if not explanation:
                last_errors = ["Model returned empty explanation text."]
                continue

            if len(explanation) < min_chars:
                last_errors = [f"Explanation too short ({len(explanation)} chars); need ≥ {min_chars}."]
                continue

            word_count = _question_word_count(explanation)
            if word_count < min_words:
                last_errors = [f"Explanation too brief ({word_count} words); need ≥ {min_words}."]
                continue

            if forbidden_terms:
                hits = _detect_forbidden_hits(explanation, forbidden_terms)
                if hits:
                    last_errors = [
